...
const AuthService = {
    ...
    async login(email, password, rememberMe = false) {
        ...
        if (DEV_BYPASS_AUTH) {
            ...
            const user = { id: 1, email, name, businessName: 'Lume MedSpa', verified: true };
            this.setToken('dev_bypass_token_' + Date.now());
            this.setCurrentUser(user);
            return { success: true, user };
        }
    ...
    // Update profile
    async updateProfile(name, businessName, settings = null) {
        ...
        try {
            const response = await fetch(`${API_BASE}/auth/profile`, {
                ...
                body: JSON.stringify({ name, businessName, settings })
            });
